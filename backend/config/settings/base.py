...
# Список учётных записей на экране входа: стенд показывают нескольким людям
# подряд, и каждому приходится вспоминать чужой логин. Пароли этот список
# не содержит и не может содержать — вход остаётся настоящим (ADR-013).
# Отдельно от `DEMO_DATA`: маркировка происхождения данных и удобство входа
# на непроизводственном стенде — разные вопросы.
DEMO_ACCOUNTS: bool = env.bool("DEMO_ACCOUNTS", default=False)
...
# Роли, которым второй фактор обязателен (BACKEND.md § 6: «настраивается»).
# Умолчание — защищённое. На демонстрационном стенде перечень пустой:
# у того, кто смотрит стенд, приложения-аутентификатора может не быть,
# а запертый снаружи администратор — не демонстрация, а тупик. Механизм
# при этом никуда не девается: привязать приложение может любой,
# и вход тогда спросит код.
TWO_FACTOR_REQUIRED_ROLES = env.list("TWO_FACTOR_REQUIRED_ROLES", default=["admin", "finance"])
