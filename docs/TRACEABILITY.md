...
| T-4.3-11 | 4.3 | Единый визуальный язык интерфейса | дизайн-система «Streamline SOC» v2: токены в `theme.ts` и `status-tokens.ts`, знак в обвязке (ADR-035) | — | все экраны | `status-tokens.test.ts` (контраст восьми пар ≥ 4.5:1), `e2e/screenshots.spec.ts` | M4 | реализовано |
| T-4.3-12 | 4.3 | Навигация по разделам системы | рельс из девяти пунктов в трёх группах, экраны раздела — вкладками | — | все экраны | `navigation.test.ts`, `e2e/screenshots.spec.ts` | M4 | реализовано |
| T-4.3-13 | 4.3 | Смена сотрудника на стенде без обхода аутентификации | `GET /auth/accounts` подставляет логин, вход настоящий (ADR-013, ADR-036) | `GET /auth/accounts` | `/login` | `accounts/tests/test_accounts_list.py` | M4 | реализовано |
| T-4.3-14 | 4.3 | Самостоятельная регистрация заказчика | подтверждение почты, затем карточка и учётная запись без участия человека (ADR-037) | `POST /auth/register`, `POST /auth/register/confirm` | `/login` | `accounts/tests/test_registration.py`, `e2e/registration.spec.ts` | M4 | реализовано |
| T-4.3-15 | 4.3 | Заявка поставщика на согласование | очередь руководителя, одобрение заводит карточку и доступ, отказ — с причиной (ADR-037) | `GET /registration-requests`, `POST /registration-requests/{id}/approve`, `.../reject` | `/admin/registrations` | `accounts/tests/test_registration.py` | M4 | реализовано |
| T-4.3-16 | 4.3 | Подтверждение адреса почты при регистрации | одноразовая ссылка, в базе хэш; доставка через общую очередь исходящих | `POST /auth/register/confirm` | `/register/confirm` | `test_the_link_works_once`, `test_wrong_token_confirms_nothing` | M4 | реализовано (доставка — заглушка до боевого SMTP) |
